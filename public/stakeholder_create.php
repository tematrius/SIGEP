...
$pageTitle = 'Ajouter une Partie Prenante';
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                ...
                (project_id, name, organization, role, type, influence, interest, email, phone, notes) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            ...
            $stakeholder_id = $pdo->lastInsertId();
            
            // Log de l'activité
            $logStmt = $pdo->prepare("
                INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description) 
                VALUES (?, 'create', 'stakeholder', ?, ?)
            ");
            $logStmt->execute([
                $_SESSION['user_id'],
                $stakeholder_id,
                "Ajout d'une partie prenante: $name"
            ]);
            
            setFlashMessage('success', 'Partie prenante ajoutée avec succès');
            redirect('stakeholders.php');
            
        } catch (PDOException $e) {
            $errors[] = 'Erreur lors de l\'ajout de la partie prenante';
        }
    }
...
<div class="row">
    <div class="col-lg-10 mx-auto">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-user-plus"></i> Ajouter une Partie Prenante</h2>
